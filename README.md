...
เมื่อต้องสแกนตรงหน้าและตั้ง `gimbal.pitch_deg: 0` DFS ใช้ `gimbal.recenter()` เพื่อจัดหัวให้ตรงกลาง ก่อนรอ yaw telemetry และ ToF scan ใหม่ที่ตรงทิศ; หน้า dashboard แสดงสถานะ `recentering` ระหว่างนี้ หากตั้ง pitch ไม่เป็นศูนย์จะใช้ `moveto()` เพื่อรักษามุม pitch ที่ตั้งไว้ DFS รอให้ SDK ยืนยันว่า action สำเร็จก่อนเริ่มสแกนทิศถัดไป แม้มุมและ ToF จะตรงเป้าก่อนแล้วก็ตาม เพื่อไม่ส่งคำสั่ง gimbal ซ้อนกัน การ recenter ไม่ใช่การรีเซ็ตพิกัดแผนที่หรือ odometry
