...
ทิศที่โค้ดแผนที่/DFS ใช้: `+X` คือด้านหน้า, `−X` คือด้านหลัง, `+Y` คือด้านขวา, `−Y` คือด้านซ้ายเมื่อเทียบกับทิศเริ่มต้น; มุมเล็ง gimbal แบบ **เทียบตัวรถ** คือ `0°` ด้านหน้า, `+90°` ฝั่ง `+Y`, `−90°` ฝั่ง `−Y`, `±180°` ด้านหลัง SDK Python 3.8 ที่ใช้ในโปรเจคส่ง `moveto(yaw=...)` ในกรอบ yaw เทียบตัวรถ (`COORDINATE_YCPN`/`yaw CAR`) แม้คำอธิบาย API บางแห่งเรียกว่าเทียบตำแหน่งตอนเปิดเครื่อง; ค่ามุมเทียบตอนเปิดเครื่องอยู่ในช่อง `yaw_ground_deg` ของ telemetry โค้ดใช้มุมเทียบตัวรถทั้งสั่งและตรวจ แล้วเลือก `+180°`/`−180°` ที่หมุนใกล้กว่าเมื่อทั้งสองแทนทิศหลังเดียวกัน มุมหมุน chassis `z` ของ SDK เป็นอีกคำสั่งหนึ่ง: คู่มือยกตัวอย่าง `+90°` หันซ้ายและ `−90°` หันขวา; การเทียบเครื่องหมาย yaw ของ telemetry กับแกนแผนที่ต้องตรวจด้วยหุ่นจริงก่อนปล่อย DFS วิ่ง

เมื่อต้องสแกนตรงหน้าและตั้ง `gimbal.pitch_deg: 0` DFS ใช้ `gimbal.recenter()` เพื่อจัดหัวให้ตรงกลาง ก่อนรอ yaw telemetry และ ToF scan ใหม่ที่ตรงทิศ; หน้า dashboard แสดงสถานะ `recentering` ระหว่างนี้ หากตั้ง pitch ไม่เป็นศูนย์จะใช้ `moveto()` เพื่อรักษามุม pitch ที่ตั้งไว้ การ recenter ไม่ใช่การรีเซ็ตพิกัดแผนที่หรือ odometry
...
`exploration.max_nodes` จำกัดจำนวนจุด DFS ต่อรอบ; `step_m`, `max_speed_m_s`, `robot_radius_m` และ `clearance_margin_m` ปรับขนาดช่อง ความเร็ว และระยะเผื่อ ไม่มีเกณฑ์ระยะ ToF สั้นสุด/ไกลสุด: ค่าบวกที่อ่านได้จะถูกใช้ตามจริง ค่า 0 หรือค่าที่ไม่ใช่ตัวเลขจะถูกปฏิเสธ ระยะใกล้ภายใน `robot_clearance_m` ยังรับเป็น scan แต่ไม่สร้างกำแพงใต้ตัวหุ่น; ระยะไกลเกินขอบแผนที่จะถูกวาดถึงขอบโดยไม่สร้างกำแพงเทียม DFS ยังตรวจพื้นที่ว่างก่อนเดิน แผนที่บันทึกอัตโนมัติที่ `data/maps/latest.json` ปุ่ม **ส่งออก JSON** ดาวน์โหลดไฟล์แบบ round-trip ที่ระบบเปิดกลับด้วยปุ่ม **เปิด JSON** ได้ ส่วน **ส่งออก ROS** ได้ ZIP ซึ่งมี `map.yaml`, `map.pgm` และ `slam.json` สำหรับนำเข้า ROS map server และเก็บ metadata เพิ่มเติม
...
ช่อง “จุดที่ควรตรวจ” แสดงข้อผิดพลาดของ run (ถ้ามี), แถว CSV ที่ตกหล่น, ช่วงที่ข้อมูลขาดนาน, ToF ช่อง 0 ที่ต่ำกว่า `review.close_tof_mm` และการเปลี่ยนสถานะ slip/impact/picked up/roll over จาก `status.csv` กดรายการที่มีเวลาเพื่อข้ามไปดูช่วงนั้น รายการนี้เป็นสัญญาณช่วยสืบเหตุ ไม่ใช่การวินิจฉัยอัตโนมัติ ค่าเริ่มต้นบันทึก position, attitude, status, ToF, battery และ gimbal ที่ความถี่ต่ำเพื่อใช้ตรวจย้อนหลัง; IMU, ESC และ adapter เปิดเพิ่มได้เมื่อต้องใช้
