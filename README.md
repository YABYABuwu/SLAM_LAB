...
DFS ใช้แผนที่เพื่อแสดงผลและบันทึกเท่านั้น การอนุญาตให้เดินใช้ scan ของ gimbal ที่หันถึงทิศและจบ action แล้ว พร้อมค่า ToF ใหม่ที่มากพอสำหรับ `step_m - sensor_offset + robot_radius_m + clearance_margin_m`; ไม่มีการให้เซลล์สิ่งกีดขวางในแผนที่ปฏิเสธทางเดิน หากไม่มีทิศที่ผ่านตั้งแต่จุดเริ่ม DFS รายงาน `no_safe_direction` บน dashboard และใน run summary/review ค่า `pivot_x_m`/`pivot_y_m` ต้องวัดจากจุดกึ่งกลางตัวหุ่นถึงแกน yaw ของ gimbal จริงเพื่อให้ระยะเผื่อมีความหมาย
