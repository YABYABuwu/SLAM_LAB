...
ระบบสำรวจใช้ `position`/`attitude` จาก RoboMaster SDK เป็น pose ตั้งต้น และใช้ ToF ช่องเดียวที่ติดบน gimbal: worker ผูกระยะกับมุม gimbal ที่ timestamp ใกล้กันแล้วฉายลำแสงหนึ่งเส้นลง occupancy grid ทุก scan ส่วน DFS หัน gimbal ดูครบ 4 ทิศ (`+X`, `−Y`, `−X`, `+Y`) ทุกครั้งก่อนเลือกเดินหรือถอยกลับ และสแกนทิศที่จะเดินซ้ำทันทีเพื่อยืนยันระยะล่าสุด แล้วใช้ `ChassisController` เดิมขับ หน้า dashboard แสดงความคืบหน้าการสแกน, แผนที่, ทิศหัว ToF, ช่อง ToF และ offset ของเซนเซอร์ ถ้า pose/ToF/gimbal/status เก่าหรือสัญญาณ status ระบุการยก ลื่น ชน หรือพลิก ระบบหยุดคำสั่งเคลื่อนที่

DFS ใช้กริดกำแพงขนาด `step_m` (ปัจจุบัน 0.60 ม.) โดยแต่ละช่องมีขอบสี่ด้านเป็น `unknown`, `open`, `wall` ขอบร่วมของช่องข้างเคียงใช้ข้อมูลเดียวกัน การสแกนที่ gimbal หยุดและ action สำเร็จแล้วจะเปิดขอบที่ลำแสงผ่านและบันทึกกำแพงที่ขอบใกล้จุดชนที่สุด DFS เลือกเดินเฉพาะขอบ `open` ที่ระยะล่าสุดเผื่อตัวหุ่นพอ แล้วสแกนยืนยันทิศนั้นอีกครั้งก่อนข้ามช่อง ดูรายละเอียดและข้อสมมติเรื่องกำแพงตามกริดใน [กริดกำแพง](docs/WALL_GRID.md)

Dashboard แสดงกริดกำแพงเป็นค่าเริ่มต้น พร้อมสถานะสี่ด้านของช่องปัจจุบัน และเลือกดู occupancy ละเอียดได้ ผลสุดท้ายของกริดบันทึกใน map JSON และ run summary; หน้า review แสดงตารางกำแพงของรันนั้น หากไม่มีขอบที่เดินผ่านได้ตั้งแต่เริ่มจะรายงาน `no_safe_direction`
...
เมื่อต้องสแกนตรงหน้า DFS ใช้ `gimbal.moveto(yaw=0)` เพื่อจัดหัวให้ตรงหน้าโดยไม่เรียก `recenter()` ที่สั่งแกน pitch เทียบ chassis ด้วย คำสั่ง `moveto()` ใช้ `gimbal.pitch_deg` ในกรอบมุมตั้งแต่เปิดเครื่อง DFS รอทั้งมุม yaw/pitch, ToF scan ใหม่ที่ตรงทิศ และ SDK action สำเร็จก่อนตรวจทางหรือส่งคำสั่ง gimbal ถัดไป หากมุมกับ ToF พร้อมก่อน action จบ หน้า dashboard จะแสดง “รอ SDK ยืนยันว่าหัวหมุนเสร็จ” ค่า `move_timeout_s` เป็นเวลาหมุนขั้นต่ำ, `action_timeout_s` เป็นเวลารอ SDK เพิ่มเติม และ `scan_timeout_s` เริ่มนับหลัง action จบ; เมื่อ timeout ระบบหยุดและบันทึก state/progress กับมุมเพื่อวิเคราะห์
...
`exploration.max_nodes` จำกัดจำนวนจุด DFS ต่อรอบ; `step_m`, `max_speed_m_s`, `robot_radius_m` และ `clearance_margin_m` ปรับขนาดช่อง ความเร็ว และระยะเผื่อ ไม่มีเกณฑ์ระยะ ToF สั้นสุด/ไกลสุด: ค่าบวกที่อ่านได้จะถูกใช้ตามจริง ค่า 0 หรือค่าที่ไม่ใช่ตัวเลขจะถูกปฏิเสธ ระยะใกล้ภายใน `robot_clearance_m` ยังรับเป็น scan แต่ไม่สร้างกำแพงใต้ตัวหุ่น; ระยะไกลเกินขอบแผนที่จะถูกวาดถึงขอบโดยไม่สร้างกำแพงเทียม DFS ใช้ระยะ ToF ใหม่ก่อนเดิน แผนที่บันทึกอัตโนมัติที่ `data/maps/latest.json` ปุ่ม **ส่งออก JSON** ดาวน์โหลดไฟล์แบบ round-trip ที่ระบบเปิดกลับด้วยปุ่ม **เปิด JSON** ได้ ส่วน **ส่งออก ROS** ได้ ZIP ซึ่งมี `map.yaml`, `map.pgm` และ `slam.json` สำหรับนำเข้า ROS map server และเก็บ metadata เพิ่มเติม
