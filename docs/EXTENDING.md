...
เมื่อ scan เป้าหมายเป็นกลางและ `gimbal.pitch_deg` เป็นศูนย์ ใช้ `recenter()` ของ SDK แล้วเผยสถานะ `recentering` ผ่าน exploration snapshot เดิม ไม่สร้างโมดูลควบคุมใหม่หรือ subscription เพิ่ม ต้องรอ action object ของ SDK สำเร็จพร้อมกับ telemetry/ToF scan ใหม่ที่ตรงทิศก่อนออกคำสั่ง gimbal ถัดไปหรือสั่ง chassis; ค่า yaw ตรงเป้าอย่างเดียวไม่หมายความว่า action ถูกปลดจาก dispatcher แล้ว หาก pitch ไม่เป็นศูนย์ให้ใช้ `moveto()` เพื่อไม่ให้ recenter เปลี่ยน pitch ที่ตั้งไว้ ก่อนเดินแต่ละครั้ง DFS ต้องสแกนครบ 4 ทิศพร้อมแสดง `scanning_1_of_4` ถึง `scanning_4_of_4` บน dashboard แล้วสแกนทิศที่เลือกซ้ำเพื่อยืนยันทางล่าสุด ใช้กฎเดียวกันเมื่อเดินย้อน stack

ToF ไม่มีเกณฑ์ระยะสั้นสุด/ไกลสุดใน config; รับค่าบวก finite ทุกค่า ค่า 0/ค่าผิดรูปแบบยังใช้ไม่ได้ ระยะภายใน `robot_clearance_m` ไม่เขียนทับพื้นที่ใต้หุ่นเป็นกำแพง; ระยะที่ไกลกว่ากริดจะถูกตัดที่ขอบแผนที่และไม่ถือว่าขอบเป็นกำแพง DFS ตรวจระยะ ToF ใหม่เทียบกับระยะเดิน รัศมีหุ่น และ margin ก่อนเรียก `ChassisController` โดยไม่ใช้ occupancy grid ปฏิเสธทางเดิน หน้า dashboard ต้องบอกว่า DFS ใช้ ToF ตัดสินใจ ส่วนแผนที่ใช้แสดงผลและ export
