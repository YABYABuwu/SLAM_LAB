...
คำสั่ง `gimbal.moveto()` เป็นมุมเทียบตอนเปิดเครื่อง (`gimbal` sample ช่องที่ 4) ขณะที่การจับคู่ลำแสงใช้มุม yaw เทียบ chassis (ช่องที่ 2) จึงห้ามส่ง relative target ตรง ๆ ให้ `moveto`; แปลงกรอบและเลือกมุมสมมูลที่อยู่ใน `[-250°,250°]` และหมุนสั้นที่สุด โดยเฉพาะทิศหลัง `+180°` กับ `−180°` ให้เพิ่มเวลาเคลื่อนตามมุมที่ต้องหมุนจริงก่อนรอ scan ใหม่
