...
- DFS ใช้ `gimbal.recenter()` เมื่อเป้าหมาย yaw อยู่ใน tolerance ของศูนย์และ pitch ที่ตั้งเป็นศูนย์; สถานะ `recentering` ปรากฏใน dashboard ผ่าน exploration snapshot จากนั้นยังต้องได้ gimbal telemetry และ ToF scan ใหม่ที่ตรงทิศก่อนตรวจทางเดิน สำหรับ pitch อื่นหรือทิศอื่นใช้ `moveto()`
