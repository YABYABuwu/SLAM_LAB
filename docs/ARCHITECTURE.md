...
- DFS ใช้ `gimbal.moveto(yaw=0)` เมื่อสแกนตรงหน้าและ `moveto()` กับทิศอื่น เพื่อไม่ให้ `recenter()` สั่ง pitch เทียบ chassis ค้าง รอ yaw/pitch telemetry และ ToF scan ใหม่ที่ตรงทิศ **และ** SDK action สำเร็จก่อนตรวจทางเดินหรือออกคำสั่ง gimbal ถัดไป แยกเวลาเคลื่อน (`move_timeout_s`), เวลารอ SDK เพิ่มเติม (`action_timeout_s`) และเวลารอ scan หลัง action (`scan_timeout_s`); dashboard แสดง `waiting_gimbal_action` เมื่อมุมและ ToF พร้อมแต่ action ยังทำงาน และ error เก็บ state/progress กับมุมเมื่อหมดเวลา
